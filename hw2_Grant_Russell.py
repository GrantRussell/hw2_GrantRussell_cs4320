...
import ir4320
MY_NAME = "Grant Russell"
MY_ANUM  = 1657874 # My UID starts with a zero, but Python doesn't like this.
MY_EMAIL = "grant.s.russ@gmail.com"

# the COLLABORATORS list contains tuples of 2 items, the name of the helper
# and their contribution to your homework
COLLABORATORS = [ 
    ('Daniel Oliveros', 'We talked about general strategy to finish assignment')
    ]
...
class BetterIndex(object):
    ...
    def wildcard_search_or(self, text):
        ...
        for word in words:
            rotated_word = self._rotate(word)
            permuterm_set = crawl_tree(self._bt.root, rotated_word)
            results = results.union(permuterm_set)

        document_results = []

        for result in results:
            document_results.append(self._documents[result])
    ...
    def wildcard_search_and(self, text):
        ...
        for word in words:
            ...
            if len(results) == 0:
                ...
                results = results.intersection(permuterm_set)
...
def main(args):
    ...
    #index.wildcard_search_and('dot cat')
